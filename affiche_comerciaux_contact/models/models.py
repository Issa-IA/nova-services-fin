        ...
        for rec in sale:
            ...
            if team_vente:
                if team_vente.crm_team_chif>0:
                    rec.x_studio_chifre_affaire_total = rec.sale_finance/team_vente.crm_team_chif
        for rec in sale:
            team_vente = False
            for record in rec.user_id.crm_team_ids:
                id = 1
            ...
            if team_vente:
                if team_vente.crm_team_N_contrat > 0:
                    rec.x_studio_nombre_contrat_tot = rec.sale_new_contrat / team_vente.crm_team_N_contrat 
        for rec in sale:
            team_vente = False
            for record in rec.user_id.crm_team_ids:
                id = 1
            ...
            if team_vente:
                if team_vente.crm_team_N_client > 0:
                    rec.x_studio_nombre_tot_client_stat = rec.sale_new_contact / team_vente.crm_team_N_client
        ########
